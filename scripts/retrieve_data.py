...
# Read SWOT nodes
def retrieve_swot_node(sword_id, start_date, end_date, logger, write=False):
    ...
    if hydrocron_response.get('results') is None or hydrocron_response['results'].get('csv') is None:
        logger.info(f"No CSV data in response for SWOT Node: {sword_id}")
        logger.info(f"Response content: {hydrocron_response}")
        return None
    
    logger.info(f"Successfully retrieved SWOT Node data for SWORD ID: {sword_id}")
    ...
    if data_swot.empty:
        logger.info(f"No data retrieved from SWOT Node: {sword_id}")
...
if __name__ == "__main__":
    ...
    # single processing examples for debugging purposes
    # results = [process_station(task) for task in tasks]
    results = [process_station(task) for task in tasks if task[1]['STATION_NU'] == '08NH127']
